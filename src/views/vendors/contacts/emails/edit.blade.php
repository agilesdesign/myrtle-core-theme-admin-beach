...
@section('page-description')
    {{ $contact->name->firstLast }} {{ $contact->title }} @ {{ $vendor->name->legal }}
@endsection
...
@section('toolbar')
    {{ Form::button('<i class="fa fa-fw fa-check"></i> Save', ['class' => 'btn btn-success', 'type' => 'submit', 'form' => 'edit-contact-email'] ) }}
    <a class="btn btn-danger" href="{{ route('admin.vendors.contacts.show', [$vendor, $contact]) }}"><i class="fa fa-fw fa-times"></i> Cancel</a>
@endsection

@section('dock')
    {!! Form::model($email, ['route' => ['admin.vendors.contacts.emails.update', $vendor, $contact, $email], 'method' => 'PATCH', 'id' => 'edit-contact-email']) !!}
